...
public class Scripts
{
    ...
    {   
        ...
        if (step == 1)
        {
            ...
            statement = configureDatabase.configConnection(username,
                    password, 1).prepareStatement(sql);
            return statement;
        } 
        else 
        {
    ...
    public LinkedList<User> getUserAccount(String password)
    {
        ...
        try
        {
            ...
            while(results.next())
            {
                user.add(new User(results.getString("USERNAME"), 
    ...
    public LinkedList<Activity> getLoginActivity(String password)
    {
        ...
        try
        {
            ...
            while (results.next())
            {
                loginHistory.add(new Activity(results.getDate("DATE").toString(), 
                                              results.getTime("TIME").toString())
                );
            }
        }
        catch (SQLException sqlErr) {
        ...
        try
        {
            ...
            while (results.next())
            {
                tickets.add(new Ticket(
                        results.getInt("TICKET_NUMBER"), 
                        results.getString("CUST_NAME"), 
                        results.getString("TITLE"), 
                        results.getString("PURCH_TIME"),
                        results.getString("PURCH_DATE"))
                );
            }
        }
        catch (SQLException sqlErr) {
        ...
        try
        {
            ...
            while (results.next())
            {
                movieList.add(new Movie(
                        ...
                        results.getString("DATE_RELEASED"), 
                        results.getBlob("COVER"), 
    ...
    public ArrayList<Detector> authenticate()
    {
        ...
        try 
        {
            ...
            while(results.next()) 
            {
                dataList.add(new Detector(results.getString("ID_NUMBER"),
                                          results.getString("PASSWORD")));
            }
        }
    ...
    {    
        ...
        try
        {
            selectMovieByDate = query(
                    "SELECT DATE_RELEASED FROM movies WHERE TITLE = ?"
            );
    ...
    public Blob getMovieCover(String title)
    {
        ...
        try
        {
            selectMovieByCover = query(
                    "SELECT COVER FROM movies WHERE TITLE = ?"
            );     
    ...
    public LinkedList<Booking> getBookings()
    {
        ...
        try
        {
            ...
            while (results.next())
            {
                bookings.add(new Booking(results.getInt("MOVIE_ID"),
                                         results.getString("CUST_NAME"), 
                                         results.getString("CONTACT_NUMBER"), 
                                         results.getInt("NUMBER_OF_TICKETS"))
                );
            }
        }
        catch (SQLException sqlErr) {
    ...
    private void insertLoginData(int loginId, String password, int userId) 
    {
        ...
        try 
        {
            insertUserLogin = query(
                    "INSERT INTO userlogin (ID,PASSWORD,USER_ID) " +
                    "VALUES(?, ?, ?)"
            );
            ...
            System.out.format("%s%n%s %d%n", 
                    "Insertion was successful.", "Row count =", count);
        } 
        catch (SQLException sqlError) {
            databaseStatusError(sqlError);
        }
    ...
    public static void databaseStatusError(Exception exception )
    {
        ...
        for (StackTraceElement error : errorStack)
        {
            System.out.printf("%-59s%-43s%-14s%-14s%n", error.getClassName(), 
                    error.getFileName(), error.getLineNumber(), 
                    error.getMethodName());
        }
    }
}